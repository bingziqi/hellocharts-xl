...
public class LineChartRenderer extends AbstractChartRenderer {
    ...
    private void drawArea(Canvas canvas, Line line) {
        ...
        final Rect contentRect = computator.getContentRectMinusAllMargins();
        float baseRawValue;
        if(line.getAreaValue() > 0){
            baseRawValue = computator.computeRawY(line.getAreaValue());
        } else {
            baseRawValue = Math.min(contentRect.bottom, Math.max(computator.computeRawY(baseValue),
                    contentRect.top));
        }
